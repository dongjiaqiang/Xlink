...
public final class Utils {
	...
	/**
	 * 从HDFS的一个路径上 将一个文件复制到HDFS另一个路径
	 *
	 * @param fs
	 * 		remote filesystem 文件系统
	 * @param appId
	 * 		application ID  应用APPID
	 * @param remoteSrcPath
	 * 		path to the remote file 远程文件地址
	 * @param homedir
	 * 		remote home directory base (will be extended)
	 * @param relativeTargetPath
	 * 		relative target path of the file (will be prefixed be the full home directory we set up)
	 ...
	 * @return Path to remote file (usually hdfs)
	 */
	static Tuple2<Path,LocalResource> setupRemoteResource(
		FileSystem fs,
		String appId,
		Path remoteSrcPath,
		Path  homedir,
		...
		FileStatus remoteFileStatus = fs.getFileStatus(remoteSrcPath);

		if(fs.isDirectory(remoteSrcPath)){
			throw new IllegalArgumentException("Remote File to copy must not be a directory: " +
				remoteSrcPath);
		}

		//copy remote resource to HDFS
		String suffix =
			".flink/"
				+ appId
				+ (relativeTargetPath.isEmpty() ? "" : "/" + relativeTargetPath);
		...
		String suffixFile = suffix + "/" + remoteSrcPath.getName();
		Path dstFile = new Path(homedir,suffixFile);

		LOG.debug("Copying remote file from {} to {}",remoteSrcPath,dstFile);

		FileUtil.copy(fs,remoteSrcPath,fs,dst,false,new Configuration());
		fs.setTimes(dstFile, remoteFileStatus.getModificationTime(), -1);
		LocalResource resource = registerLocalResource(dstFile,remoteFileStatus.getLen(),remoteFileStatus.getModificationTime());
		//remote dst of jar file
		return Tuple2.of(dstFile,resource);
	}
